...
eval_reg_value = lambda x, B : np.linalg.norm(B@x, 1)
...
for beta in beta_list:
    ...
    x = cp.Variable(n)
    objective = cp.Minimize(cp.norm(x-y_delta) + beta*cp.norm(B@x, 1))
    constraints = []
...
#beta_star = 12.9
beta_star = 20.5651
d_star = 68.27047155841665

x = cp.Variable(n)
objective = cp.Minimize(cp.norm(x-y_delta) + beta_star*cp.norm(B@x, 1))
constraints = []
prob = cp.Problem(objective, constraints)
result = prob.solve(solver="SCS")
...
for alpha in [1/3, 1, 3]:
    x = cp.Variable(n)
    objective = cp.Minimize(cp.norm(B@x, 1))
...
titles = ["Solution with beta_star = 20.5651",
